...
네트워크 대역폭(기본 1분 수집)과 정적 파일 전송 순위(10분 집계)도 제공합니다. 인터페이스·Nginx 로그 연결과 지표 해석은 [네트워크 지표 운영 문서](../doc/NETWORK_METRICS.md)를 참고합니다.

서버 하드웨어 패널은 CPU 사용률·평균 부하, 메모리 사용률·사용량, 디스크별 여유 공간과 agent-client의 CPU·메모리를 1분마다 갱신합니다.
...
`/api/metrics/hardware`는 백그라운드에서 1분마다 수집한 최신 값만 반환합니다. 대시보드를 여러 개 열거나 수동 새로고침해도 추가 수집하지 않으며, 과거 이력은 저장하지 않습니다. OS 통계 조회는 게임 처리와 분리된 작업에서 실행하고, 디스크 용량은 파일 순회 없이 파일시스템 통계로 읽습니다. 수집이 3분 넘게 지연되면 마지막 값과 지연 안내를 표시합니다.

- 서버 CPU는 논리 코어 전체 기준 0~100%, agent-client CPU는 코어 1개를 100%로 표시하므로 100%를 넘을 수 있습니다. 둘 다 직전 수집 구간 평균이며, 시작 후 첫 1분은 측정 대기로 표시합니다.
- 평균 부하는 1·5·15분의 OS load average이며 사용률(%)과 다릅니다. 지원하지 않는 OS에서는 생략합니다.
- 서버 메모리는 전체 RAM에서 사용 가능한 메모리(재사용 가능한 캐시 포함)를 뺀 값입니다. 디스크는 마운트별 전체 용량과 일반 사용자에게 남은 용량을 표시합니다.
- agent-client는 같은 실행 환경에서 보이는 `agent-client` / `agent-client.exe` 프로세스와 모든 자식 프로세스(LLM CLI 포함)의 합계입니다. 스레드를 별도 프로세스로 더하지 않습니다. 메모리는 RSS 합계이므로 공유 페이지가 중복될 수 있습니다. 프로세스를 찾지 못하면 미감지 상태를 표시합니다.
- 운영 systemd 구성은 같은 호스트의 agent-client를 자동 수집합니다. 컨테이너에서는 OS가 노출하는 CPU·메모리·마운트 및 PID namespace 범위를 따르며, 컨테이너 제한량 기준 지표가 아닙니다. 별도 컨테이너나 다른 호스트의 agent-client는 수집하지 않습니다. Linux의 NFS·CIFS·autofs 마운트는 조회 대상에서 제외합니다.

### 게임 지표

동시 접속 그래프는 최근 1일을 분별 관측값으로, 1주일·1개월은 시간별 평균, 6개월은 6시간 평균, 1년은 일평균으로 표시합니다. 웹 접속과 외부 에이전트의 인원 또는 평균 구성을 색상으로 구분합니다. 현재 접속 카드와 그래프 툴팁에서 유형별 계정 수·비율을 확인할 수 있으며, 구분 정보가 없는 과거 기록은 기타·미분류로 표시합니다. 긴 기간 그래프에는 최고·최저값이나 최고값 보조선을 표시하지 않으며, 기간 최고 카드는 별도로 유지합니다.
...
서버 총 골드 그래프는 기존 `gold_snapshots`의 시간별 합계를 사용합니다. 오프라인 캐릭터와 NPC를 포함한 모든 캐릭터의 보유 골드에 영지 세금 계좌 잔액을 더해 집계하며, 최근 총량과 마지막 집계 기준 시각을 표시합니다. 1일·1주일·1개월은 시간별 기록, 6개월은 6시간 평균, 1년은 일평균입니다. 새로고침은 저장된 집계를 조회하며 추가 기록을 생성하지 않습니다.
...
동시 접속·유니크 계정·총 골드·1인당 골드 그래프에는 서버 배포 시각을 세로 점선으로 표시합니다. 서버가 시작할 때 기록한 빌드 해시가 직전 시작과 다를 때만 배포로 봅니다.

1인당 골드 아래의 상인 물가 지수 그래프는 상인 회의마다 조정되는 소비재 구매가 지수를 계단선으로 보여줍니다. 현재 지수와 마지막 회의를 요약하고, 회의 점에서 조정 전후 지수와 1인당 골드 변화를 확인합니다. 1주일·1개월·6개월·1년을 선택하며 기본 100%를 점선으로 표시합니다.

그 아래에는 골드 생산 순위 표가 있습니다. 상인 판매는 아이템별로 표시하고, 던전 보상 상자·동전 더미 획득(몬스터·일반 상자·파괴물 등)·동전 주머니 개봉·주민 NPC 급여도 생산원별로 합산합니다. 실제 지급액 내림차순으로 수량 또는 횟수·생성 골드·전체 생산액 대비 비중을 표시합니다. 흥정과 지갑 상한 적용 후 실제 지급액을 기록하고, 동전 더미는 주웠을 때 집계합니다. 주민 NPC·플레이어 간 거래와 팁은 제외하며 소비한 골드는 차감하지 않습니다. 기본 1일이며 1일·1주일·1개월·6개월·1년을 독립적으로 선택합니다. 메모리에서 시간·생산원별로 합산해 다음 정각에 저장하며 완료된 시간대만 표시합니다. 로그아웃 후에도 집계를 유지하고 정상 종료 시 진행 중인 시간대까지 저장하지만, 비정상 종료 시 미저장 통계는 유실될 수 있습니다. 기존 판매 기록은 보존하며 판매 외 골드는 적용 이후부터 수집하고 별도 시작 시각을 안내합니다.
...
레벨 순위 아래에는 같은 형식의 골드 상위 10명 표와 골드 변화 그래프를 표시합니다. 공식 NPC를 제외한 전체 캐릭터를 보유 골드 내림차순, 동점이면 캐릭터 ID 오름차순으로 정렬합니다. 같은 계정의 캐릭터 표시, 색상과 선택 강조, 1주일·1개월·6개월·1년 조회 및 1시간 갱신 방식은 레벨 순위와 같습니다. 금액은 `8g 61s 34c`처럼 표시하며 레벨 그래프와 조회 기간을 독립적으로 선택합니다.
...
골드 순위 아래에는 현재 **+7 이상 무기를 보유한 캐릭터 전원**의 표와 변화 그래프가 있습니다. 인원 제한 없이 최고 인챈트 내림차순으로 표시하며 등수는 표시하지 않습니다. 가방·장비 슬롯의 무기를 기준으로 하므로 장착 여부와 무관하며 방어구·창고 아이템은 제외합니다. 공식 NPC 제외, 같은 계정 표시, 기간 선택·선 강조·1시간 갱신은 다른 순위와 같습니다. 이력에는 +7 미만이던 시기의 값도 포함하지만 현재 최고값이 +7 미만으로 내려가면 표와 그래프에서 제외됩니다. 인원이 많으면 표와 범례를 스크롤할 수 있고, 10명을 초과할 때는 캐릭터를 선택해 툴팁 상세값을 확인합니다.

무기 순위 바로 아래의 인챈트 실패 내역 표는 같은 캐릭터·무기 종류·강화 단계의 소멸 기록을 한 줄로 합쳐 누적 실패 횟수를 표시합니다. 수집 이후 전체 기록을 합산한 뒤 최근 실패 시각순으로 최대 10개 항목을 보여주며, 다른 무기나 강화 단계는 별도 행으로 남깁니다. 예를 들어 같은 캐릭터가 `+8 철검`을 3번 잃으면 `+8 → +9 · 3회` 한 줄로 표시합니다. 최근 실패 시각(KST)·그때의 캐릭터명·무기명·강화 시도를 함께 보여줍니다. 캐릭터 ID로 묶으므로 이름이 바뀌어도 횟수가 이어집니다. 공식 NPC와 방어구 실패는 제외합니다. 신규 적용 이후부터 수집하며 과거 실패는 추정하지 않습니다. 사건을 32초 주기의 저장 작업과 정상 종료 시 DB에 저장하고, 화면은 1시간마다 또는 수동 새로고침 시 조회합니다. 저장 오류는 다음 저장 때 재시도하며 로그아웃 후에도 대기 기록을 유지하지만, 비정상 종료 시 아직 저장하지 않은 기록은 유실될 수 있습니다. 이력의 이름과 무기 정보는 실패 당시 값을 보존합니다.

실패 내역 아래에는 같은 형식의 방어구 인챈트 상위 10명 표와 변화 그래프가 있습니다. 가방·장비의 방어구를 아이템 정의의 슬롯별로 묶어 최고 인챈트를 하나씩 고른 뒤 합산합니다. 투구·갑옷·방패·장갑·바지·신발을 포함하며 같은 슬롯의 중복 방어구는 더하지 않습니다. 무기·액세서리·창고 아이템은 제외하고 방어구가 없으면 `+0`입니다. 예를 들어 투구 최고 `+4`, 갑옷 최고 `+3`, 방패 최고 `+2`이면 합계 `+9`입니다. 장착 여부는 순위에 영향을 주지 않으며 기록·조회·갱신 방식은 무기 순위와 같습니다.

DB와 API의 금액은 코퍼 단위이며 `1골드 = 100실버 = 10,000코퍼`입니다. 순위 표, 총 골드·1인당 골드 요약, 그래프 축과 툴팁은 `8g 61s 34c` 형식으로 표시하고 각 단위에 금색·은색·구리색을 적용합니다. 0인 단위는 생략하고 전체 금액이 0이면 `0c`를 표시합니다. 평균값은 가장 가까운 1코퍼로 반올림합니다.

동시 접속 수는 서버 시작 시와 이후 60초마다 수집하며, API 요청 시의 현재 접속 수도 기록합니다. 같은 분의 마지막 관측값을 저장하고 최고값은 기간 최고 카드용으로 별도 보존합니다. 정상 종료 시에는 종료 시각의 0명을 별도로 기록해 같은 분 안에 재시작해도 남습니다. 최근 1일 그래프에는 해당 시각의 0명을 표시하며 긴 기간에서는 평균에 포함합니다. 분별 원본은 24시간이 지나도 삭제하거나 시간별 기록으로 대체하지 않고 조회할 때만 집계합니다. 현재 접속 카드와 추이는 매분 5초 후 갱신합니다. 그 외 지표는 서버 시작 시와 매 정각에 수집하고 화면은 매 정각 5초 후 갱신합니다. 최초 접속·기간 변경·수동 새로고침은 즉시 조회합니다. 유니크 계정 집계는 하루 한 번이며 접속 중 마지막 확인 시각은 시간마다 저장합니다. 캐릭터 이력은 매시간 변경된 값만 추가하고 32초 게임 저장에서는 추가하지 않습니다. 기존 시간·분 단위 이력은 보존하며 과거에 놓친 최고값은 소급하지 않습니다.
...
영지 순위 아래의 **시그네의 영웅담 원장**은 시그네가 읽는 원장의 날짜·주인공·기록 내용을 최근 추가된 줄부터 모두 표시합니다. 날짜·주인공·내용을 검색할 수 있고, 매시간 또는 상단 새로고침 버튼으로 원장 수정을 반영합니다. 사실과 공연 방향을 원문 그대로 보여주며, 현재 공연에서 선택된 곡 목록은 아닙니다. 파일이 없거나 비어 있는 상태와 읽기 오류를 구분하고, 잘못된 형식으로 건너뛴 줄 수를 안내합니다.

원장 API는 `/api/metrics/heroic-tales`이며 기존 운영자 인증을 적용합니다. 서버는 기본적으로 `agent-client/data/tales/ledger.txt`를 요청마다 읽습니다. 다른 위치를 사용하는 경우 서버의 `--tales-ledger` / `TALES_LEDGER`를 시그네가 사용하는 원장 파일로 지정합니다. 원장 내용은 프런트엔드 빌드에 포함하지 않습니다.

영웅담 원장 아래의 **전투 기록 추적 대상**에는 서버에 적용된 대상의 현재 캐릭터 이름·ID·인원수를 표시합니다. 오프라인 캐릭터도 포함하며 삭제되었거나 존재하지 않는 ID는 이름 확인 불가로 표시합니다. `/api/metrics/combat-audit-targets`를 매분 또는 수동 새로고침 시 조회하며 기존 운영자 인증을 적용합니다. 대상 파일 변경은 서버의 기존 10분 재조회 주기에 반영됩니다. 설정 방법은 [전투 감사](../doc/COMBAT_AUDIT.md)를 참고하세요.
