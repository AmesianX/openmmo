...
- `client/public/portraits/grida.webp` — ORKEA 점원 Grida의 상점 창 상반신 초상화. OpenAI Codex built-in ImageGen, ChatGPT Pro x20 등급(사용자 확인), 생성 2026-09-20. 프로젝트용 생성 이미지이며 OpenAI 서비스 약관의 출력물 이용 조건에 따른다. [기존 원화](../images/characters/grida-concept.png)를 얼굴·머리·의상 참조로 사용했고 원화 출처와 라이선스는 [캐릭터 문서](characters.md#grida--그리다--orkea-점원-2026-09-20)에 기록되어 있다. 생성 원본은 `../images/characters/grida-portrait.png`(1254² RGBA PNG)로 보관. 배포본은 원본 상단 2/3(1254×836)을 크롭한 512×341 WebP q88이며 실제 투명 alpha를 유지한다(`crop=1254:836:0:0,scale=512:341:flags=lanczos`, 2026-09-20 수정). `traderId=grida`로 자동 연결. [실제 생성 프롬프트](grida-portrait-prompt.txt).

- `client/public/portraits/tobin.webp` — 낚시꾼 Tobin의 상점 창 초상화. OpenAI Codex built-in ImageGen 편집, ChatGPT Pro 20x, 2026-09-21. 프로젝트용 생성 이미지이며 OpenAI 서비스 약관의 출력물 이용 조건에 따른다. [기존 토빈 원화](../images/characters/tobin-concept.png)의 얼굴·표정·모자·복장을 유지해 가슴까지의 구도로 다듬고 배경을 제거했다. 원화 출처와 라이선스는 [캐릭터 문서](characters.md#tobin--토빈--강가의-낚시꾼-2026-09-21) 참조. 가장자리 정리 편집을 거친 최종 생성 원본은 [tobin-portrait.png](../images/characters/tobin-portrait.png)(1254² RGBA PNG)로 보관한다. 게임용은 512² WebP q88, alpha와 비율 유지(`scale=512:512:flags=lanczos`). `traderId=tobin`으로 자동 연결. [실제 편집 프롬프트](tobin-portrait-prompt.txt).
