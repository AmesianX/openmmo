...
- `client/public/portraits/grida.webp` — ORKEA 점원 Grida의 상점 창 상반신 초상화. OpenAI Codex built-in ImageGen, ChatGPT Pro x20 등급(사용자 확인), 생성 2026-09-20. 프로젝트용 생성 이미지이며 OpenAI 서비스 약관의 출력물 이용 조건에 따른다. [기존 원화](../images/characters/grida-concept.png)를 얼굴·머리·의상 참조로 사용했고 원화 출처와 라이선스는 [캐릭터 문서](characters.md#grida--그리다--orkea-점원-2026-09-20)에 기록되어 있다. 생성 원본은 `../images/characters/grida-portrait.png`(1254² RGBA PNG), 배포본은 512² WebP q88이며 실제 투명 alpha와 원본 비율을 유지한다. `traderId=grida`로 자동 연결. [실제 생성 프롬프트](grida-portrait-prompt.txt).
